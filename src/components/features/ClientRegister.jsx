...
export default function ClientRegister() {
  ...
  const groupSamplesByCase = (samples) => {
    ...
    samples.forEach(sample => {
      // Only group samples that have actual case numbers (new format)
      // Dummy data without case_id will be treated as individual samples
      const caseKey = (sample.case_number && sample.case_id) ? sample.case_number : `INDIVIDUAL_${sample.id}`;
      if (!grouped[caseKey]) {
        grouped[caseKey] = [];
      }
                ...
                {samples.length === 0 && !loading ? (
                  ...
                  (() => {
                    ...
                    Object.entries(groupedSamples).forEach(([caseNumber, caseSamples]) => {
                      // Check if this is a real case group (new format) or individual sample (dummy data)
                      const isRealCase = caseNumber.startsWith('CASE_');
                      const isIndividualSample = caseNumber.startsWith('INDIVIDUAL_');
                      
                      // Add case header row only for real case groups with multiple samples
                      if (isRealCase && caseSamples.length > 1) {
                        rows.push(
                          <TableRow key={`case-${caseNumber}`} sx={{ backgroundColor: 'rgba(13, 72, 143, 0.05)' }}>
                            <TableCell colSpan={7} sx={{ fontWeight: 'bold', color: 'primary.main', fontSize: '0.9rem' }}>
                              📁 Case: {caseNumber} ({caseSamples.length} samples)
                      ...
                      caseSamples.forEach((sample, index) => {
                        // Only apply grouping styles to real case groups, not individual dummy samples
                        const isGrouped = isRealCase && caseSamples.length > 1;
                        rows.push(
                          <TableRow 
                            key={sample.id} 
                            hover 
