...
function createBatches() {
  ...
  // PCR batches (for samples in pcr_batched stage)
  const pcrBatches = [
    { batch_number: 'JDS_2001', operator: 'John Smith', pcr_date: '2024-01-15', status: 'completed' },
    { batch_number: 'JDS_2002', operator: 'Jane Doe', pcr_date: '2024-01-16', status: 'active' },
    { batch_number: 'JDS_2003', operator: 'Mike Johnson', pcr_date: '2024-01-17', status: 'active' }
  ];
  
  // Electrophoresis batches (for samples in electro_batched stage)
  const electroBatches = [
    { batch_number: 'ELEC_3001', operator: 'Sarah Wilson', electro_date: '2024-01-16', status: 'completed' },
    { batch_number: 'ELEC_3002', operator: 'Tom Brown', electro_date: '2024-01-17', status: 'active' }
  ];
  ...
  pcrBatches.forEach(batch => {
    try {
      ...
      batches.push({ id: db.lastInsertRowid, ...batch, type: 'pcr' });
    } catch (error) {
  ...
  electroBatches.forEach(batch => {
    try {
      ...
      batches.push({ id: db.lastInsertRowid, ...batch, type: 'electro' });
    } catch (error) {
...
function createManualSamples() {
  const insertCase = db.prepare(`
    INSERT OR IGNORE INTO cases (case_number, case_type, status)
    VALUES (?, ?, ?)
  `);
  ...
  const insertSample = db.prepare(`
    INSERT INTO samples (
      ...
      collection_date, submission_date, reason, sample_type, 
      user_id, workflow_status
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  
  // Get batch IDs for assignment
  const pcrBatches = db.prepare('SELECT id, batch_number FROM batches WHERE batch_number LIKE ?').all('JDS_%');
  const electroBatches = db.prepare('SELECT id, batch_number FROM batches WHERE batch_number LIKE ?').all('ELEC_%');
  
  let sampleCount = 0;
  const startNumber = 50000; // Start from a high number to avoid conflicts
  ...
  Object.entries(SAMPLE_DISTRIBUTION).forEach(([stage, count]) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const caseNumber = `CASE-${Math.floor((startNumber + sampleCount) / 3).toString().padStart(5, '0')}`;
      
      // Create case if needed
      try {
        insertCase.run(caseNumber, 'paternity', 'active');
      } catch (error) {
        // Case might already exist, that's fine
      }
      
      // Get case ID
      const caseRow = db.prepare('SELECT id FROM cases WHERE case_number = ?').get(caseNumber);
      const caseId = caseRow ? caseRow.id : null;
      
      // Determine sample type based on position in case
      ...
      const sampleData = {
        ...
        reason: `Paternity testing - ${sampleType}`,
        sample_type: sampleType,
        user_id: faker.helpers.arrayElement([1, 2, 3]),
      ...
      try {
        ...
        // Assign to batches if in batched stages
        if (stage === 'pcr_batched' && pcrBatches.length > 0) {
          const batch = pcrBatches[i % pcrBatches.length];
          // Could update sample with batch info if we had a batch_id column
        }
        if (stage === 'electro_batched' && electroBatches.length > 0) {
          const batch = electroBatches[i % electroBatches.length];
          // Could update sample with batch info if we had a batch_id column
        }
      } catch (error) {
...
async function main() {
  ...
  try {
    ...
    console.log('\n🏗️ Ensuring database structure...');
    ensureCasesTable();
    
    // Clear existing manual samples (keep AUTO- samples)
